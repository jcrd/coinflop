...
export default function Logger(history, name = "logger", func = console.log) {
  const signals = {}
  ...
  signals[Signals.Round.BetAction.Placed] = ({
    betPlaced,
    epoch,
    direction,
    amount,
    ...
    func(`Round ${epoch}: Placing ${direction} bet...`)
    if (betPlaced) {
      if (simulate) {
  ...
  signals[Signals.Round.Close] = (round) => {
    const entry = history.values[round.epoch]
