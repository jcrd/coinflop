...
export default function (contract, signerAddress) {
  ...
  function betBullHandler(sender, epoch, amount) {
    emitter.emit(Signals.Round.Event.BetBull, sender, epoch, amount)
  }

  function betBearHandler(sender, epoch, amount) {
    emitter.emit(Signals.Round.Event.BetBear, sender, epoch, amount)
  }
  ...
  function addObserver(observer) {
    for (const signal in observer) {
      emitter.on(signal, observer[signal])
    }
  }

  function removeObserver(observer) {
    for (const signal in observer) {
      emitter.off(signal, observer[signal])
    }
  }
  ...
  function useStrategy(s) {
    const setStrategy = () => {
      if (strategy) {
        removeObserver(strategy.observer())
      }
  ...
  return {
    run: async (...window) => {
      running = true
      contract.on("BetBull", betBullHandler)
      contract.on("BetBear", betBearHandler)
      while (running) {
        if (!(await emitSignals(...window))) {
          break
        }
      }
      contract.off("BetBull", betBullHandler)
      contract.off("BetBear", betBearHandler)
    },
    abort: () => {
      controller.abort()
      running = false
