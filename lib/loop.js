...
export default function (contract, signerAddress) {
  ...
  emitter.on(Signals.Round.BetPlaced, ({ betPlaced, simulate }) => {
    if (!simulate && !betPlaced) {
      running = false
    }
  })
  ...
  function useStrategy(s) {
    const setStrategy = () => {
      ...
      strategy.run(
        (data) => emitter.emit(Signals.Round.BetPlaced, data),
        (data) => emitter.emit(Signals.Broadcast, data)
      )
