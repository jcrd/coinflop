...
export default class Loop extends Emitter {
  constructor(contract, signerAddress) {
    ...
    this.emitter.on(
      ...
      ({ betPlaced, simulate, error }) => {
        if (!simulate && !betPlaced && error !== null) {
          running = false
        }
      }
    )
    ...
    this.strategyObserver = {
      signals: {
        [StrategySignals.PlaceBet]: (data) =>
          this.emitter.emit(Signals.Round.BetAction.Placed, data),
        [StrategySignals.SkipBet]: (data) =>
          this.emitter.emit(Signals.Round.BetAction.Skipped, data),
